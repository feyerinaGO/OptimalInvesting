...
class MarriedPut2(QCAlgorithm):
    ...
    def Initialize(self):
        ...
        self.options_alloc = 1 # 1 option for X num of shares (balanced would be 100)
    ...
    def VIXRank(self):
        ...
        # (Current - Min) / (Max - Min)
        self.rank = ((history["close"][-1] - history["open"][-1]) / (max(history["high"]) - min(history["low"])))
