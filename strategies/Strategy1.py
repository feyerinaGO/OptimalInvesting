...
class MarriedPut1(QCAlgorithm):
    ...
    def Initialize(self):
        ...
        self.options_alloc = 40 # 1 option for X num of shares (balanced would be 100)
    ...
    def VIXRank(self):
        ...
        # (Current - Min) / (Max - Min)
        self.rank = ((self.Securities[self.symbol].Price - min(history["low"])) / (max(history["high"]) - min(history["low"])))
